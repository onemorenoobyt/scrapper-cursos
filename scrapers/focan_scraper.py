...
def _normalize_date(date_string):
    ...
    try:
        # Intenta parsear la fecha en formato día/mes/año
        dt_object = datetime.strptime(date_string, '%d/%m/%Y')
        return dt_object.strftime('%Y-%m-%d')
    except ValueError:
        # Si falla, devuelve la cadena original o un valor por defecto
...
def scrape():
    ...
    if not course_list:
        print(f"No se encontró el contenedor de cursos en {CENTRO_NOMBRE}.")
        return []
